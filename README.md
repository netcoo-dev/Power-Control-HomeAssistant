...
**Bug fix:**
- Corretto bug critico nel ripristino carichi: l'accesso al dizionario `device_off` usava l'indice numerico del loop (`repeat.index - 1`) invece della chiave entity_id (`repeat.item`). Questo causava il mancato controllo della potenza prima della riaccensione dei dispositivi.
- Aggiunto `restore: true` all'`input_boolean.check_distacco` per preservare lo stato tra i riavvii di Home Assistant.
- Corrette parentesi nella macro `dict_device()` del template `real_active` per rendere esplicita la precedenza degli operatori `and`/`or`.
- Corretti errori tipografici nel codice: alias automazione, CSS (`--primary-text-color`), valori colore con spazi finali (`"white"`, `"OliveDrab"`), proprietà CSS non valida (`font-color` → `color`), `border: 100px` → `border: none`.

**Nuove funzionalità:**
- **Modalità Test** (`input_boolean.power_control_test_mode`): quando attiva, il sistema invia le notifiche e simula il distacco/ripristino senza spegnere né accendere realmente i dispositivi. Utile per verificare la configurazione. Accessibile dal popup Impostazioni nella card.
- **Validazione configurazione soglie**: nuova automazione che notifica l'utente se le soglie non sono coerenti (`distacco_urgente` deve essere > `distacco_carico`; `ripristino_carico` deve essere < `distacco_carico`).

**Miglioramenti grafici card:**
- Il valore watt istantaneo cambia colore dinamicamente: verde (<70% soglia), arancione (70–90%), rosso (≥90%).
- Il grafico mostra due linee di riferimento tratteggiate per le soglie di distacco (arancione) e urgente (rosso), lette dinamicamente dagli `input_number` configurati.
- La linea del grafico cambia colore in base alla prossimità alla soglia (`color_threshold`).
- I bottoni inferiori della card ora mostrano la label testuale (Lista, Impostazioni, Push, Media).
- Il badge di stato mostra "Modalità Test attiva" (blu) quando la modalità test è abilitata.

**Per aggiornare sostituire i file:** `control_power.yaml`, `power_control.jinja` e `card.txt`.